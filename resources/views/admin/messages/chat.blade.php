...
<div class="container mx-auto px-4 py-8 max-w-3xl">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Chat with {{ $intern->name }}</h1>
        <a href="{{ route('admin.messages.index') }}" class="text-blue-600 hover:underline">Back to Message</a>
    </div>

    <!-- Intern Details -->
    <div class="bg-white shadow-sm rounded-lg p-4 mb-6">
        <p class="text-lg font-medium text-gray-700">Name: {{ $intern->name }}</p>
        <p class="text-lg text-gray-600">Email: {{ $intern->email }}</p>
    </div>
    ...
    <form id="messageForm" class="flex gap-3">
        ...
        <input type="text" name="message" id="messageInput" placeholder="Type your message..."
               class="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2好处
...
<script>
    ...
    const fetchUrl = "{{ route('admin.messages.fetch', ['intern_id' => $intern->id]) }}";
    const storeUrl = "{{ route('admin.messages.store') }}";
